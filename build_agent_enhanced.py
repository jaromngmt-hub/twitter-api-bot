...
class EnhancedBuildAgent:
    ...
    async def analyze_tweet(self, tweet_text: str, username: str) -> Optional[ProjectRequirements]:
        ...
        try:
            ...
            # Force buildable if AI is unsure - user explicitly clicked BUILD
            if not data.get("is_buildable"):
                logger.warning(f"AI said not buildable: {data.get('reason')}. Forcing buildable=True because user explicitly requested BUILD.")
                data["is_buildable"] = True
                data["project_name"] = data.get("project_name", "AutoGenerated Project")
                data["description"] = data.get("description", tweet_text[:100])
                data["problem_statement"] = data.get("problem_statement", "Automate the described workflow")
                data["target_users"] = data.get("target_users", ["Users who need this automation"])
                data["core_features"] = data.get("core_features", ["Main functionality from tweet"])
                data["appetite"] = data.get("appetite", "2 weeks")
                data["constraints"] = data.get("constraints", [])
                data["success_criteria"] = data.get("success_criteria", ["Works as described"])
